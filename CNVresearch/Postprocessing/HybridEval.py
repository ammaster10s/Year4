...
def hybrid_post_process_expert(image_gray, deeplab_mask, morph_mask, transition_y=0.45):
    ...
    final_output = morphology.binary_erosion(final_output, morphology.disk(2)).astype(np.uint8)
...
def boundary_f1_score(y_true, y_pred, tolerance=3):
    """
    Calculates F1 score based on boundaries allowing a specific pixel tolerance.
    Tolerance=3 pixels handles human hand jitter.
    """
    # Handle empty masks
    if np.sum(y_true) == 0 and np.sum(y_pred) == 0: return 1.0
    if np.sum(y_true) == 0 or np.sum(y_pred) == 0: return 0.0

    # Get Boundaries (Mask - Erode = Edge)
    gt_boundary = np.logical_xor(y_true, binary_erosion(y_true))
    pred_boundary = np.logical_xor(y_pred, binary_erosion(y_pred))

    # Distance Transforms
    # dist_to_gt: Distance from every pixel to nearest GT edge
    dist_to_gt = distance_transform_edt(np.logical_not(gt_boundary))
    # dist_to_pred: Distance from every pixel to nearest Pred edge
    dist_to_pred = distance_transform_edt(np.logical_not(pred_boundary))

    # Precision: How much of Pred edge is close to GT?
    pred_edge_pixels = pred_boundary.sum()
    if pred_edge_pixels == 0: precision = 0
    else:
        dists = dist_to_gt[pred_boundary]
        precision = np.sum(dists <= tolerance) / pred_edge_pixels

    # Recall: How much of GT edge is close to Pred?
    gt_edge_pixels = gt_boundary.sum()
    if gt_edge_pixels == 0: recall = 0
    else:
        dists = dist_to_pred[gt_boundary]
        recall = np.sum(dists <= tolerance) / gt_edge_pixels
